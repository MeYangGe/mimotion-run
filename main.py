...
def execute():
    ...
    if len(user_list) == len(passwd_list):
        ...
        print(summary)
        push_to_push_dt(push_results, summary)
    else:
...
if __name__ == "__main__":
    ...
    if os.environ.__contains__("CONFIG") is False:
        ...
        try:
            ...
            traceback.print_exc()
            exit(1)
        PUSH_DT_TOKEN = config.get('PUSH_DT_TOKEN')
        PUSH_DT_HOUR = config.get('PUSH_DT_HOUR')
        PUSH_DT_MAX = get_int_value_default(config, 'PUSH_DT_MAX', 30)
